...
void loop() {
  ...
  // Movimiento siempre con velocidad fija
    ...
    else if (comando == 'i') {    //sin servo
    girarIzquierda();
    }
    else if (comando == 'd') {  //sin servo
    girarDerecha();
    }
...
void girarIzquierda() {
  ...
  // Motor izquierdo lento, derecho rápido
  int pwm1 = constrain((int)(A_M1 * (vDeseada * 0.1) + B_M1), 0, 255);
  int pwm2 = constrain((int)(A_M2 * (vDeseada * 1.5) + B_M2), 0, 255);
...
void girarDerecha() {
  ...
  // Motor izquierdo rápido, derecho lento
  int pwm1 = constrain((int)(A_M1 * (vDeseada * 1.2) + B_M1), 0, 255);
  int pwm2 = constrain((int)(A_M2 * (vDeseada * 0.3) + B_M2), 0, 255);
